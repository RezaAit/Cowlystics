...
// Source: 8 real verified BD cattle (online platform)
// ═══════════════════════════════════════════════════════

// Market rates — calibrated from 20 real Dhaka platform cattle (2026 Eid)
// All breeds: ৳490-540/kg. Deshi/premium upper band, Friesian lower band.
...
// Qurbani Pricing — rate-anchored formula (৳490-545/kg real data, n=20)
// OLD multiplier chain: 480×1.12×1.10×1.05 = ৳621/kg → 25% inflated
// NEW: base rate band + flat ৳/kg adders → stays within real ৳490-545/kg
...
function getEidPremium(weightKg: number): number {
  if (weightKg <= 200) return 0;       // small cattle: online price already includes costs
...
// Based on systematic Gemini underestimation analysis
// ═══════════════════════════════════════════════════════

// Calibration — single combined factor (new accurate Gemini prompt needs minimal correction)
// Validated against 20 real Dhaka cattle (2026):
//   Friesian: still underestimated due to dairy body illusion → 1.40-1.55x
//   Local Large Cross large (480kg+): prompt accurate → 1.02x
//   Local Large Cross medium (350-480kg): prompt slightly overshoots → 0.96x
//   Local/LocalCross: prompt accurate → 0.96-1.00x
function getCombinedFactor(breed: string, rawMid: number): number {
  if (breed === "Friesian Cross") {
    if (rawMid >= 380) return 1.55; // 600-700kg actual — massive dairy frame illusion
    if (rawMid >= 280) return 1.40; // 400-500kg actual
    return 1.25;
  }
  ...
  if (breed === "Local Large Cross") {
    if (rawMid >= 480) return 1.02;  // very large — slight upward
    if (rawMid >= 350) return 0.96;  // large — prompt overshoots slightly
    return 0.95;
  }
  // Local / Local Cross
...
// Gemini confidence ignored — we compute from signal quality
// Target range: 65–90%
// ═══════════════════════════════════════════════════════
...
function calculateConfidence(
  ...
  breed:       string,
  visualConf:  string,         // "low" | "medium" | "high" from Gemini
  sellerClaim: number | undefined
): ConfidenceResult {
  // Base from Gemini visual quality
  let score = { high: 78, medium: 68, low: 56 }[visualConf] ?? 68;

  // Raw range tightness — tighter = Gemini was more certain
  ...
  // Calibration factor — small correction = AI was already close to truth
  ...
  else                score -= 16;  // Friesian large correction = less confident
  ...
  if (sellerClaim !== undefined) {
    ...
    if      (diff <= 0.07) score += 8;   // within 7% — strong validation
  ...
  const breedPenalty: Record<string, number> = {
    "Local":               +2,   // most distinctive
    "Local Cross":          0,
    "Local Large Cross":   -2,
    "Friesian Cross":      -5,   // often misclassified
    "Sahiwal Cross":       -3,
    "Brahman Cross":       -2,
    "Sindhi Cross":        -3,
    "Premium Cross":       -4,
...
// ═══════════════════════════════════════════════════════
// SECTION 6 — GEMINI PROMPT (v3 — accuracy optimized)
// Changes from v2:
//   + Full breed list (9 breeds, matches new GEMINI_PROMPT doc)
//   + Dark cattle rule: size-based 5-20% (not flat 8-12%)
//   + Friesian floor: weight_min NEVER below 380 if visually large
//   + Removed "small cattle overestimated" — replaced with large cattle rule
//   + Tight spread rules from user's prompt doc
//   + visual_confidence replaces numeric % (backend calculates score)
// ═══════════════════════════════════════════════════════
...
DARK CATTLE RULE (black/dark grey coat):
- Small dark frame (<250 kg visually): NO dark coat adjustment — judge purely on body size.
- Medium dark frame (250–400 kg visually): +8–12% upward correction.
- Large dark frame (400 kg+ visually): +12–18% upward correction.
- Do NOT inflate if frame is clearly compact and small.
...
176 kg=small desi | 235 kg=avg desi | 260 kg=compact dark Local Cross (no coat inflation) |
...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json({
      ...
      // Estimate A: Live rate model (real data)
      estimate_a: {
      ...
      // Debug (auto-removed in production)
      ...(process.env.NODE_ENV !== "production" && {
        _debug: {
          ...
          qurbani_formula:    `৳${qurbani.base_price_per_kg}/kg × ${qurbani.breed_multiplier}(breed) × ${qurbani.beauty_multiplier}(beauty) × ${qurbani.health_multiplier}(health) + ৳${qurbani.eid_premium} = ৳${qurbani.final_price}`,
        },
      }),
    });
